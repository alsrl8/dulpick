...
function placeOption(place) {
  const rating = place.rating ? `<span class="place-rating">★ ${escapeHtml(place.rating)}${place.reviewCount ? ` <small>(${escapeHtml(place.reviewCount)})</small>` : ''}</span>` : ''
  const reservation = place.reservation ? `<span class="reservation-badge">${escapeHtml(place.reservation)}</span>` : ''
  return `<button type="button" class="place-option" data-place="${escapeHtml(place.id)}"><span class="place-thumb">${place.image ? `<img src="${escapeHtml(place.image)}" alt="">` : '<i>둘픽</i>'}</span><span class="place-content"><span class="place-heading"><b>${escapeHtml(place.name)}</b>${reservation}</span><span class="place-meta">${escapeHtml([place.area, place.menu].filter(Boolean).join(' · '))}</span>${place.description || place.reason ? `<span class="place-description">${escapeHtml(place.description || place.reason)}</span>` : ''}${rating}</span><span class="place-arrow">›</span></button>`
}
...
function bindStep(key) {
  ...
  document.querySelector('#from-dulpick')?.addEventListener('click', async () => {
    ...
    try {
      ...
      results.innerHTML = places.length ? places.map(placeOption).join('') : '<p>이 지역에 저장된 후보가 아직 없어요.</p>'
